...
async function apiFetch(url, options = {}) {
  ...
  try {
    ...
    throw new Error(
      "Network request failed. Please verify the backend API URL or server availability.",
    );
  }
...
export const fetchTrendingNews = (limit = 5) =>
  apiFetch(`/news/trending?limit=${limit}`);
export const fetchMostReadNews = (limit = 5) =>
  apiFetch(`/news/mostread?limit=${limit}`);
export const fetchLatestNews = (limit = 6) =>
  apiFetch(`/news/latest?limit=${limit}`);
export const fetchBreakingNews = () => apiFetch("/news/breaking");
export const fetchNewsByCategory = (name, sort) =>
  apiFetch(
    `/news/category/${encodeURIComponent(name)}${sort ? `?sort=${sort}` : ""}`,
  );
...
export const deleteNews = (id) => apiFetch(`/news/${id}`, { method: "DELETE" });
